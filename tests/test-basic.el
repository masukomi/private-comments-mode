...
(ert-deftest pcm-equal-signs ()
  ; We need the server to be running in order to
  ; test our interactions with it.
  (skip-unless (executable-find private-comments-executable))
  ...
  (let* ((repo (expand-file-name "tests/pcm-equal-signs"
         ...
         (file-name (expand-file-name "test-repo.c"))
         (buffer (find-file-noselect file-name)))
    ;; create a repo and file to work with.
    (vc-git-create-repo)
    (with-current-buffer buffer
    ...
    ; PCM Edit is a buffer generated by private-comments-mode
    ; when you tell it to record something.
    ; IF this fails with "No buffer named PCM Edit" it means that PC
    ; has failed to generated the new buffer.
    ;
    (with-current-buffer "PCM Edit"
      (insert "=====")
      (call-interactively #'private-comments-edit-done))
